...
is_hardcourt <- any(grepl("Cincinnati|Montreal|Washington|US Open|Winston",tournaments,ignore.case=TRUE))
day_label <- format(Sys.Date(),"%B %d")
tourney_header <- if(is_hardcourt) "Wimbledon" else paste(sapply(tournaments[1:min(2,length(tournaments))],tour_short),collapse=" & ")
tourney_tag <- if(is_hardcourt) "#USOpen #CincinnatiOpen" else "#Tennis #ATP"
...
# POST 4: Promo
promo_post <- paste(c("🎾 US Open season is here.","","tennisIQ gives you:","→ Daily model predictions for every match","→ Matchup DNA — visual breakdown of every contest","→ Surface Elo, serve profiles, fatigue signals","→ Upset alerts before the upsets happen","","Free through US Open — no code needed.",SITE_URL,"",paste0(tourney_tag," #TennisIQ")),collapse="
")
...
instagram_post <- if(nrow(top3)>0) local({
  ...
  lines<-c(lines,"Full Matchup DNA at link in bio 👆","",if(PROMO_ACTIVE) paste0("Free through US Open 🎾") else NULL,"","#USOpen #CincinnatiOpen #Tennis #TennisAnalytics #TennisData #TennisIQ #ATP #WTA")
  paste(lines,collapse="
")
}) else NULL
